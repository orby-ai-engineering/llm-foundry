...
# Original file: https://github.com/mosaicml/llm-foundry/blob/main/scripts/train/train.py
import copy
import gc
import logging
import os
...
from llmfoundry.data.dataloader import build_multimodal_dataloader
...
CUSTOM_TASK_TYPES = ["contract_entity_extraction", "action_understanding"]
...
def build_composer_model(model_cfg: DictConfig):
    ...
    return COMPOSER_MODEL_REGISTRY[model_cfg.name](model_cfg)
...
def main(cfg: DictConfig) -> Trainer:
    ...
    if fsdp_config and deepspeed_config:
        raise ValueError("both fsdp and deepspeed cannot be used together")
    ...
    # Build Image Processor for Llava
    if "llava" in model_config["name"]:
        processor = LlavaNextProcessor.from_pretrained(model_config['pretrained_model_name_or_path'])
        
    else:
        raise RuntimeError("Non llava models are not supported right now.")
    ...
    if eval_loader_config is not None:
        ...
        for eval_config in eval_configs:
            eval_dataloader = build_multimodal_dataloader(
                eval_config, processor, device_eval_batch_size
            )
    ...
    ###### >>>>> ORBY MODIFIED CODE STARTS <<<<<<<< ######
    extracted_tasks_config = None
    icl_tasks_list: ListConfig = om.create([])
    if isinstance(icl_tasks_config, str):
        log.info(f"Extracting ICL task config from path: {icl_tasks_config}")
        with open(icl_tasks_config, "r") as icl_f:
            extracted_tasks_config = om.load(icl_f)
        icl_tasks_list = extracted_tasks_config.icl_tasks  # type: ignore[union-attr]
    else:
        if icl_tasks_config is not None:
            icl_tasks_list = icl_tasks_config.icl_tasks  # type: ignore[union-attr]

    if extracted_tasks_config is not None:
        default_icl_tasks = om.create([])
        custom_icl_tasks = om.create([])
        # Find the custom and default ICL tasks from config
        for icl_cfg in icl_tasks_list:
            if icl_cfg.icl_task_type in DEFAULT_TASK_TYPES:
                default_icl_tasks.append(icl_cfg)
            elif icl_cfg.icl_task_type in CUSTOM_TASK_TYPES:
                custom_icl_tasks.append(icl_cfg)

        # Create evaluators which are supported by default. Not using
        # eval gauntlet callback here so that default and custom metrics are
        # computed together.
        icl_evaluators, _, _ = build_icl_data_and_gauntlet(
            default_icl_tasks,
        ...
        # Create evaluators for Orby
        # Not using eval gauntlet callback here so that default
        # and custom metrics are
        # computed together.
        # TODO(sanjari): Add support for custom evaluators back
        """ 
        (
            custom_evaluators,
            custom_logger_keys,
        ) = build_custom_icl_data(
            custom_icl_tasks,
            tokenizer,
            device_eval_batch_size,
            icl_seq_len if icl_seq_len else max_seq_len,
        ...
        # Create the callback only once to aggregate all the metrics together
        # correctly

        eval_gauntlet_callback = build_combined_icl_gauntlet(
            icl_evaluators,
            icl_logger_keys,
            custom_evaluators,
            custom_logger_keys,
        ...
        """
    ###### >>>>> ORBY MODIFIED CODE ENDS <<<<<<<< ######

    # Build Model
    print("Initializing model...")
    with init_context:
        model = build_composer_model(model_config)
    ...
    # TODO(sanjari): Fix trainer evaluation 
