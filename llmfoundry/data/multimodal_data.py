...
def build_multimodal_finetuning_dataloader(cfg: DictConfig,
    ...
    if cfg.dataset.get('remote') is not None or streams is not None:
        ...
        # TODO(sanjari): Ensure that data collation works when 
        # multiple resolutions are added
        collate_fn, dataloader_batch_size = _build_collate_fn(
            cfg, text_tokenizer, device_batch_size, processor)
        ...
        raise RuntimeError("Only support for streaming datasets has been added for multimodal models.") 
...
def _validate_config(dataset_cfg: DictConfig) -> None:
    ...
    if dataset_cfg.get('hf_name') is not None:
        ...
        illegal_keys = ['local', 'remote', 'streams']
    ...
    elif dataset_cfg.get('remote') is not None or dataset_cfg.get('streams') is not None:
        ...
        if discovered_illegal_keys:
            raise ValueError(
                'The dataset config sets a value for `remote` or `streams` as well as the ' +\
                f'following keys: {", ".join(discovered_illegal_keys)}.\n' +\
        ...
        if dataset_cfg.get('local') is None and dataset_cfg.get('streams') is None:
            raise ValueError(
...
def _build_collate_fn(
    ...
    if packing_ratio is None:
        ...
        print("Using device batch size: ", device_batch_size)
...
if __name__ == '__main__':
    ...
    cfg = om.create({
        'dataset': {
            'remote':
                's3://orby-llm/datasets/streaming/multimodal_mind2web_mix_direct_p4n1_fullimg_n_rational_p1n2_crop_img-att3/train',
            'local':
                'test_data/',
            'packing_ratio':
                None,
    ...
    device_batch_size = 1
    dataspec = build_multimodal_finetuning_dataloader(cfg, processor,
                                             device_batch_size)
    dataloader = dataspec.dataloader
    # print(dataspec.get_num_tokens_in_batch())
    ...
    for i, batch in enumerate(dataloader):
        ...
        # print("pixel values "
              # , [batch['image_sizes'][i].shape for i in range(4)])
        for j in range(1):
            print(f'--- Sample {j} ---')
            if cfg.dataset.decoder_only_format:
                if packing:
                    for subseq in range(int(batch['sequence_id'][j].max()) + 1):
                        ...
                        print(
                            '\033[93m{}\033[00m\n'.format('INPUT IDS:'),
                            processor.decode(batch['input_ids'][
                                j,
                                torch.logical_and(
